...
import seq2seq
from seq2seq.dataset.prepare_dataset import random_batch, random_captions
import sys
import numpy as np
import collections
import os
...
class Generator(object):
    ...
    def generate(self, encoder, decoder, data, keys):
        ...
        if self.output_opt == 1:
            ...
            labels = np.ones((len(keys), 3)) * -1
            bboxes = np.ones((len(keys), 3, 4)) * -1
        ...
        for batch_index in range(steps_per_epoch): #steps_per_epoch
            if (batch_index+1) % self.display_step == 0:
                print('%d / %d'%(batch_index, steps_per_epoch))
                
            input_variables, input_lengths = random_captions(self.batch_size, data, self.dev_cap_lang, 
            ...
            decoder_outputs, xy_gmm_params, wh_gmm_params, decoder_hidden, other = \
                decoder(encoder_hidden, encoder_outputs,  is_training=0, 
            ...
            if len(ls) > 1:
                ...
                if self.output_opt == 1:
                    ...
                    for i in range(len(ls)):
                        ...
                        if i < 3:
                            labels[batch_index,i] = ls[i]
                            bboxes[batch_index,i] = [xs[i], ys[i], ws[i], hs[i]]
        ...
        if self.output_opt == 1:
            ...
            with open('%s/bboxes.npy'%(self.expt_dir), 'wb') as f:
                np.save(f, bboxes)
            with open('%s/labels.npy'%(self.expt_dir), 'wb') as f:
                np.save(f, labels)
