...
def prepare_gen_data(dev_path, mean_std_path, max_len, min_len, 
    train_cap_word2index, train_cap_index2word, train_label_word2index, 
    train_label_index2word, dev_filename_path):
    ...
    # Read the file and split into lines
    lines = open(dev_path).read().strip().split('\n')
    keys = list(range(len(lines)))

    # Split every line into tuples and normalize captions
    dev_tuples, xs, ys, ws, hs = [], [], [], [], []
    line_num = 0
    for line in lines:
        if line_num % 50000 == 0:
            print('loading {}/{}'.format(line_num, len(lines)))
        ...
        str_cap = line
        str_cap = normalize_string(str_cap)
        dev_tuples.append([str_cap])
        line_num += 1

    dev_cap_lang = Lang('caption')
    dev_label_lang = Lang('label')
...
def random_captions(batch_size, tuples, cap_lang, label_lang, x_mean_std, 
    y_mean_std, w_mean_std, r_mean_std, is_training=0, select_index=None):
    cap_seqs, label_seqs, x_seqs, y_seqs, w_seqs, r_seqs = [], [], [], [], [], []
    #print(tuples)
    if is_training:
    ...
    #print(cap_seqs)
    # Zip into pairs, sort by length (descending), unzip
    cap_seqs = sorted(cap_seqs, key=lambda p: len(p), reverse=True)
    #print(cap_seqs)
    # For input and target sequences, get array of lengths and pad with 0s to max length
    cap_lengths = [len(s) for s in cap_seqs]
    cap_padded = [pad_seq(s, max(cap_lengths), 0) for s in cap_seqs]
