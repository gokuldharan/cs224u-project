...
from seq2seq.dataset.prepare_dataset import prepare_data, prepare_test_data, prepare_gen_data, \
    random_batch, get_class_sta
from seq2seq.evaluator import Evaluator, Generator
...
parser.add_argument('--load_checkpoint', action='store', dest='load_checkpoint', 
    default='../data/pretrained/box_ckpt', help='The name of the checkpoint to load, \
...
parser.add_argument('--count_smooth', type=float, default=1e5, 
    help='The balancing parameter for box loss.')
parser.add_argument('--is_training', type=int, default=0, help='The state for training or test')
...
parser.add_argument('--output_opt', type=int, default=1, help='The output option (0/1)')
parser.add_argument('--embedding_dim', type=int, default=256, help='The embedding dimension')
parser.add_argument('--encoder_path', type=str, 
    default='../models/coco/text_encoder100.pth', 
    help='encoder path.')
...
if opt.is_training:
    ...
    logging.info("loading checkpoint from {}".format( opt.load_checkpoint))
    ...
    keys = prepare_gen_data(opt.dev_path, opt.mean_std_path, opt.max_len, opt.min_len, 
        cap_word2index, cap_index2word, label_word2index, label_index2word, opt.dev_filename_path)


    #for k in label_index2word:
    #    print(str(k) + ": " + label_index2word[k])

    #for k in dev_cap_lang.index2word:
    #    print(str(k) + ": " + dev_cap_lang.index2word[k])

    #evaluator = Generator(opt.batch_size, opt.early_stop_len, opt.expt_dir, dev_cap_lang, 
    #    dev_label_lang, x_mean_std, y_mean_std, w_mean_std, r_mean_std, gaussian_dict, 
    #    opt.box_saving_folder, opt.output_opt)
    #evaluator.generate(encoder, decoder, dev_tuples, keys)
